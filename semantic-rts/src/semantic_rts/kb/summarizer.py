...
Uses SUMMARIZER_V3: single call returns summary + condition +
tested_methods + concepts + tier + topology_scope.
"""
...
_VALID_TOPOLOGY = {"unit", "integration", "system"}
...
def summarize_test(
    ...
    """Return (summary, concepts, tier, tested_methods, condition, topology_scope).
    ...
    prompt = SUMMARIZER_V3_TEMPLATE.format(
    ...
    for attempt in range(2):
        ...
        try:
            result = client.chat(prompt, version_tag=SUMMARIZER_V3)
            ...
            if parsed:
                ...
                topology_scope = topology_raw if topology_raw in _VALID_TOPOLOGY else "unit"
                if summary:
                    return summary, concepts, tier, tested_methods, condition, topology_scope
    ...
    return f"Tests {tm.class_simple}.{tm.method}", [], 3, [], "", "unit"
